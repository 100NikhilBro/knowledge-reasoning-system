 ...
 */
export function canonicalizeEntityKey(type, value) {
    const raw = value.trim();
    if (!raw) {
        return "";
    }
    const prefix = `${type.toLowerCase()}:`;
    const withoutPrefix = raw.toLowerCase().startsWith(prefix)
        ? raw.slice(prefix.length)
        : raw;
    switch (type) {
        case "Proposal": {
            const match = withoutPrefix.match(/pep[\s_-]*(\d+)/i) ??
 ...
 */
export function buildGraphId(type, value) {
    const key = canonicalizeEntityKey(type, value);
    const prefix = `${type.toLowerCase()}:`;
    if (!key) {
